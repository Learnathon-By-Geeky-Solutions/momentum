...
from langgraph import graph, node  # Ensure you have installed langgraph via pip
dotenv.load_dotenv()
...
def intent_extraction(user_message: str) -> dict:
    ...
    response = openai.ChatCompletion.create(
         model="gpt-4",
         messages=[
             {"role": "system", "content": "You are an assistant that extracts user intent for an e-commerce platform."},
...
def product_search(query: str, db: Session) -> str:
    ...
    for product in products:
         response_text += f"- {product.product_name} (${product.price}) [ID: {product.product_id}]\n"
