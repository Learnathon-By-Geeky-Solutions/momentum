...
class AuthUtils:  
    def verify_google_token(self, token: str):  
        try:  
            ...
            if id_info["iss"] not in ["accounts.google.com", "https://accounts.google.com"] or id_info["aud"] != GOOGLE_CLIENT_ID:  
...
async def send_verification_email(email: str, token: str):  
    ...
    await send_email("Verify Your Email", email, f"Click the link to verify your email: {link}")  
  
async def send_reset_email(email: str, link: str):  
    await send_email("Reset Your Password", email, f"Click the link to reset your password: {link}")  
...
def verify_token(token: str = Security(oauth2_scheme)):  
    try:  
        ...
        raise HTTPException(status.HTTP_401_UNAUTHORIZED, detail=INVALID_TOKEN, headers={"WWW-Authenticate": "Bearer"})  
